...
def stock(product_id: str, request: Request):
    stock_value = random.choice([0, 2, 5, 10])
    log_event("info", "stock_checked", request_id=request.headers.get("x-request-id", "-"), path=f"/stock/{product_id}", method="GET", status_code=200, product_id=product_id, stock=stock_value)
    return {"product_id": product_id, "stock": stock_value}
